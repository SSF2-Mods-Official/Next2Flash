...
class N2DBuilder:
    ...
    def _collect_priority_char_ids(self, tags: List[SWFTag]) -> Set[int]:
        """Collect SWF character IDs transitively referenced by the main timeline.

        Walks all frames of the main timeline, collects every char_id from
        PlaceObject actions, then recursively follows DefineSprite definitions
        to gather all transitively referenced characters.
        """
        frames = analyze_timeline(tags, self.header['version'])
        direct: Set[int] = set()
        for frame in frames:
            for action in frame.actions:
                if isinstance(action, PlaceAction) and action.char_id is not None:
                    direct.add(action.char_id)
            for sa in frame.sounds:
                direct.add(sa.sound_id)

        # BFS: follow sprites to find all transitive references
        visited: Set[int] = set(direct)
        queue = list(direct)
        while queue:
            cid = queue.pop()
            if self.char_types.get(cid) == 'container' and cid in self.sprite_data:
                _, nested_tags = self.sprite_data[cid]
                sub_frames = analyze_timeline(nested_tags, self.header['version'])
                for sf in sub_frames:
                    for action in sf.actions:
                        if isinstance(action, PlaceAction) and action.char_id is not None:
                            if action.char_id not in visited:
                                visited.add(action.char_id)
                                queue.append(action.char_id)
                    for sa in sf.sounds:
                        if sa.sound_id not in visited:
                            visited.add(sa.sound_id)
                            queue.append(sa.sound_id)
        return visited

    def build_skeleton(self, tags: List[SWFTag]):
        """Build N2D with metadata only — no bitmap/shape/sound decoding.

        Creates library entries with dimensions, bounds, and type info but
        zero decoded pixel data. Every heavy asset is marked lazy=True with
        its swfCharId so the editor can fetch decoded data on-demand via
        /api/lazy/asset/<charId>.

        Texts, fonts, and containers are still fully built (they're fast).
        """
        log.info('build_skeleton: building skeleton library entries')
        t0 = time.time()
        step = lambda msg: print(f"  [{time.time()-t0:6.1f}s] {msg}", flush=True)

        if not hasattr(self, 'frame_scripts'):
            self.frame_scripts = {}
        ...
        # ── Bitmaps — metadata only, no RGBA decode ──
        bitmap_count = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'bitmap':
                continue
            ...
            sym_name = self.symbol_names.get(cid, '')
            display_name = sym_name.split('.')[-1] if sym_name else f"Bitmap_{cid}"
            width, height = self.bitmap_dims.get(cid, (1, 1))
            entry = {
                'id': lid,
                'swfCharId': cid,
                'name': display_name,
            ...
            self.libraries.append(entry)
            bitmap_count += 1
        step(f"Bitmaps: {bitmap_count} (all lazy)")

        # ── Shapes — bounds only, no recodes parse ──
        shape_count = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'shape':
                continue
            ...
            self.libraries.append(entry)
            shape_count += 1
        step(f"Shapes: {shape_count} (all lazy)")

        # ── Morph shapes — bounds only ──
        morph_count = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'morphShape':
                continue
            ...
            entry = {
                ...
                'bounds': {'xMin': 0, 'xMax': 20, 'yMin': 0, 'yMax': 20},
                'endRecodes': [],
                'endBounds': {'xMin': 0, 'xMax': 20, 'yMin': 0, 'yMax': 20},
                'lazy': True,
            }
            if cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
            ...
            self.libraries.append(entry)
            morph_count += 1
        step(f"Morph shapes: {morph_count} (all lazy)")

        # ── Sounds — metadata only, no audio decode ──
        sound_count = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'sound':
                continue
            ...
            self.libraries.append(entry)
            sound_count += 1
        step(f"Sounds: {sound_count} (all lazy)")

        # ── Texts (always parsed — fast) ──
        text_count = 0
        text_parsed = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'text':
            ...
            if cid in self.raw_tag_data:
                ...
                if tag_type == TAG_DEFINE_EDIT_TEXT:
                    ...
                    try:
                        parsed_props = parse_define_text(
                            full_data, tag_type, self.font_names,
                            self.font_attrs, self.font_code_tables)
                        text_parsed += 1
                    except Exception:
                        pass
            if parsed_props is not None:
                if '_boundsOffset' in parsed_props:
                    ...
                    self.text_bounds_offsets[lid] = tuple(offset)
                entry = {
                    'id': lid, 'swfCharId': cid, 'name': display_name,
                    'type': 'text', 'symbol': sym_name,
                    'folderId': self.folder_ids.get('text', 0),
                }
                entry.update(parsed_props)
            else:
                entry = {
                    'id': lid, 'swfCharId': cid, 'name': display_name,
                    'type': 'shape', 'symbol': sym_name,
                    'folderId': self.folder_ids.get('text', 0),
                    'bitmapId': 0, 'grid': None, 'inBitmap': False,
                    'recodes': [], 'bounds': {'xMin': 0, 'xMax': 20, 'yMin': 0, 'yMax': 20},
                    'lazy': True,
                }
            if cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
            ...
            self.libraries.append(entry)
            text_count += 1
        step(f"Texts: {text_count} ({text_parsed} parsed)")

        # ── Fonts (always built — fast) ──
        font_count = 0
        _font_aux_by_cid: Dict[int, List[Tuple[int, bytes]]] = {}
        for tt, body in self.global_raw_tags:
            if tt in (73, 74, 88) and len(body) >= 2:
        ...
        for cid in all_char_ids:
            ...
            entry = {
                'id': lid, 'swfCharId': cid, 'name': display_name,
                'type': 'shape', 'isFont': True, 'symbol': sym_name,
                'folderId': self.folder_ids.get('font', 0),
                'bitmapId': 0, 'grid': None, 'inBitmap': False,
                'recodes': [], 'bounds': {'xMin': 0, 'xMax': 20, 'yMin': 0, 'yMax': 20},
                'lazy': True,
        ...
        for cid in all_char_ids:
            ...
            self.libraries.append(container)
            sprite_count += 1
        step(f"Containers: {sprite_count}")

        elapsed = time.time() - t0
        total = len(self.libraries)
        step(f"Skeleton build complete in {elapsed:.2f}s — "
             f"{total} entries (all bitmaps/shapes/sounds lazy)")

    def build_priority(self, tags: List[SWFTag]):
        """Build N2D with full data for main-timeline assets, stubs for the rest.

        This is the seamless lazy-load path. Assets visible on the main timeline
        (and transitively through sprites) are fully decoded — bitmaps get RGBA,
        shapes get parsed recodes, sounds get audio buffers. Everything else gets
        lightweight stubs with rawTagBody preserved for later export/save.

        No re-load is needed: the editor shows real rendered content immediately.
        Full conversion of remaining assets happens only at save/export time.
        """
        log.info('build_priority: building priority library entries')
        t0 = time.time()
        step = lambda msg: print(f"  [{time.time()-t0:6.1f}s] {msg}", flush=True)
        ...
        # Collect which char IDs are needed for the visible main timeline
        priority_ids = self._collect_priority_char_ids(tags)
        step(f"Priority chars: {len(priority_ids)} of {len(self.char_types)}")

        # Allocate n2d library IDs for all SWF characters (full ID space —
        # consistent whether priority or not, so export works)
        self.swf_to_n2d[0] = 0
        self.next_lib_id = 1
        all_char_ids = sorted(self.char_types.keys())
        for cid in all_char_ids:
        ...
        # Build bitmap ID mapping (needed for shape parsing)
        bitmap_id_map: Dict[int, int] = {}
        for cid in all_char_ids:
            if self.char_types.get(cid) == 'bitmap':
                bitmap_id_map[cid] = self.swf_to_n2d.get(cid, 0)
        ...
        for cid in all_char_ids:
            ...
            buffer_str = ''
            is_lazy = cid not in priority_ids

            # Priority bitmaps: full RGBA decode
            if not is_lazy and cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
                rgba = b''
                if tag_type in (TAG_DEFINE_BITS_LOSSLESS, TAG_DEFINE_BITS_LOSSLESS2):
                    dw, dh, rgba = decode_lossless_to_rgba(tag_type, body)
            ...
            self.libraries.append(entry)
            bitmap_count += 1
        step(f"Bitmaps: {bitmap_count} total, {decode_ok} priority decoded")
        ...
        for cid in all_char_ids:
            ...
            # Priority shapes: full binary parse
            if not is_lazy and cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
                if tag_type in (TAG_DEFINE_SHAPE, TAG_DEFINE_SHAPE2,
                                TAG_DEFINE_SHAPE3, TAG_DEFINE_SHAPE4):
            ...
            # Gray rectangle placeholder for unparsed or lazy shapes
            if not recodes:
                x0 = bounds.get('xMin', 0)
                y0 = bounds.get('yMin', 0)
                x1 = bounds.get('xMax', 100)
            ...
            self.libraries.append(entry)
            shape_count += 1
        step(f"Shapes: {shape_count} total, {shape_parsed} priority parsed")

        # ── Phase 3: Morph shapes ────────────────────────────────────
        morph_count = 0
        morph_parsed = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'morphShape':
            ...
            # Priority morph shapes: full parse
            if not is_lazy and cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
                try:
                    recodes, bounds, end_recodes, end_bounds, has_bitmap_fill = \
            ...
            self.libraries.append(entry)
            morph_count += 1
        step(f"Morph shapes: {morph_count} total, {morph_parsed} priority parsed")
        ...
        for cid in all_char_ids:
            if self.char_types[cid] != 'sound':
                continue
            if cid in priority_ids and cid in self.raw_tag_data:
                tag_type, body = self.raw_tag_data[cid]
                fmt_name, audio_bytes, swf_rate = extract_sound_buffer(body)
                sound_data_cache[cid] = (fmt_name, audio_bytes, swf_rate)
                if fmt_name == 'nellymoser' and audio_bytes:
        ...
        # Convert Nellymoser→MP3 in parallel for priority sounds
        nelly_results = {}
        if nelly_jobs and _find_ffmpeg():
            max_workers = min(8, len(nelly_jobs))
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for cid in all_char_ids:
            ...
            buffer_str = ''
            if not is_lazy and cid in sound_data_cache:
                fmt_name, audio_bytes, swf_rate = sound_data_cache[cid]
                if cid in nelly_results:
                    audio_bytes = nelly_results[cid]
                if audio_bytes:
                    buffer_str = base64.b64encode(
            ...
            self.libraries.append(entry)
            sound_count += 1
        step(f"Sounds: {sound_count} total, {sound_decoded} priority decoded")

        # ── Phase 5: Texts (always parsed — fast) ────────────────────
        text_count = 0
        text_parsed = 0
        for cid in all_char_ids:
            if self.char_types[cid] != 'text':
        ...
        # ── Phase 6: Fonts (always built — fast) ─────────────────────
        font_count = 0
        _font_aux_by_cid: Dict[int, List[Tuple[int, bytes]]] = {}
        for tt, body in self.global_raw_tags:
            if tt in (73, 74, 88) and len(body) >= 2:
        ...
        step(f"Priority build complete in {elapsed:.2f}s — "
             f"{total} entries ({priority_count} fully decoded, "
             f"{total - priority_count} lazy stubs)")
