...
import { Event, ProgressEvent as Next2DProgressEvent, IOErrorEvent, HTTPStatusEvent } from "@next2d/events";
/**
 ...
 * @public
 */
export const execute = async (loader, event) => {
    ...
    if (loaderInfo.willTrigger(Next2DProgressEvent.PROGRESS)) {
        loaderInfo.dispatchEvent(new Next2DProgressEvent(Next2DProgressEvent.PROGRESS, false, event.loaded, event.total));
    }
    // http status event
    if (loaderInfo.willTrigger(HTTPStatusEvent.HTTP_STATUS)) {
        const responseHeaders = $headerStringToArray(target.getAllResponseHeaders());
        loaderInfo.dispatchEvent(new HTTPStatusEvent(HTTPStatusEvent.HTTP_STATUS, false, target.status, target.responseURL, responseHeaders));
    }
    if (199 < target.status && 400 > target.status) {
        ...
        if (loaderInfo.willTrigger(IOErrorEvent.IO_ERROR)) {
            loaderInfo.dispatchEvent(new IOErrorEvent(IOErrorEvent.IO_ERROR, false, target.statusText));
        }
    }
};
