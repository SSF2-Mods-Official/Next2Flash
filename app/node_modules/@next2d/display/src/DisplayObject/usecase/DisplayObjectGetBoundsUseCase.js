 ...
 * @protected
 */
export const execute = (display_object, target_display_object = null) => {
    const matrix = display_object.concatenatedMatrix;
    const rawBounds = displayObjectGetRawBoundsUseCase(display_object);
    // to global
    const bounds = displayObjectCalcBoundsMatrixService(rawBounds[0], rawBounds[1], rawBounds[2], rawBounds[3], matrix.rawData);
    // pool
    ...
    const targetMatrix = target_display_object.concatenatedMatrix;
    targetMatrix.invert();
    const targetBounds = displayObjectCalcBoundsMatrixService(bounds[0], bounds[1], bounds[2], bounds[3], targetMatrix.rawData);
    // pool
    $poolBoundsArray(bounds);
    Matrix.release(targetMatrix.rawData);
    const rectangle = new Rectangle(targetBounds[0], targetBounds[1], Math.abs(targetBounds[2] - targetBounds[0]), Math.abs(targetBounds[3] - targetBounds[1]));
