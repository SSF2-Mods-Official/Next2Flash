 ...
 * @protected
 */
export const execute = (x_min, y_min, x_max, y_max, matrix) => {
    const m0 = matrix[0];
    const m1 = matrix[1];
    const m2 = matrix[2];
    const m3 = matrix[3];
    ...
    return $getBoundsArray(Math.min(x0, x1, x2, x3), Math.min(y0, y1, y2, y3), Math.max(x0, x1, x2, x3), Math.max(y0, y1, y2, y3));
};
