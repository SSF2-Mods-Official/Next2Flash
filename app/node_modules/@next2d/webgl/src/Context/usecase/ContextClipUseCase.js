...
import { $clipLevels, $clipBounds } from "../../Mask";
import { $gl, $context } from "../../WebGLUtil";
import { $fillBufferIndexes, $clearFillBufferSetting } from "../../Mesh";
...
export const execute = () => {
    ...
    // レベルと描画範囲をセット
    const bounds = $clipBounds.get(currentAttachmentObject.clipLevel);
    const xMin = bounds[0];
    const yMin = bounds[1];
    const xMax = bounds[2];
    const yMax = bounds[3];
    ...
    $gl.scissor(xMin, currentAttachmentObject.height - yMin - height, width, height);
    const vertexArrayObject = vertexArrayObjectCreateFillObjectUseCase();
    let level = $clipLevels.get(currentAttachmentObject.clipLevel);
    let offset = 0;
    let gridData = null;
    ...
    for (let idx = 0; idx < length; idx++) {
        ...
        shaderManagerFillUseCase(shaderManager, vertexArrayObject, offset, indexCount);
        offset += indexCount;
        ++level;
        if (level > 7) {
