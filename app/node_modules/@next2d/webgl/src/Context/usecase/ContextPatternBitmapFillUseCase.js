...
export const execute = (vertex_array_object, offset, index_count, grid_data) => {
    ...
    // bitmap setting
    const pixels = $bitmapData.shift();
    const matrix = $bitmapData.shift();
    const width = $bitmapData.shift();
    const height = $bitmapData.shift();
    const repeat = $bitmapData.shift();
    const smooth = $bitmapData.shift();
    ...
    shaderManagerSetBitmapFillUniformService(shaderManager, width, height, grid_data);
