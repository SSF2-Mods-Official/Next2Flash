...
import { $gl, $linearGradientXY, $inverseMatrix, $context, $poolFloat32Array6, $poolFloat32Array4 } from "../../WebGLUtil";
/**
 ...
 * @protected
 */
export const execute = (vertex_array_object, offset, index_count, grid_data) => {
    const stops = $gradientData.shift();
    const matrix = $gradientData.shift();
    const spread = $gradientData.shift();
    const interpolation = $gradientData.shift();
    ...
    shaderManagerFillUseCase(coverageShader, vertex_array_object, offset, index_count);
    $gl.disable($gl.SAMPLE_ALPHA_TO_COVERAGE);
    $gl.stencilFunc($gl.NOTEQUAL, 0, 0xff);
    $gl.stencilOp($gl.KEEP, $gl.ZERO, $gl.ZERO);
    $gl.colorMask(true, true, true, true);
    const shaderManager = variantsGradientShapeShaderUseCase(false, false, spread, useGrid);
    const points = $linearGradientXY(matrix);
    const inverseMatrix = $inverseMatrix($context.$matrix);
    shaderManagerSetGradientFillUniformService(shaderManager, 0, $context.$matrix, inverseMatrix, 0, points, grid_data);
    $poolFloat32Array4(points);
    $poolFloat32Array6(inverseMatrix);
    shaderManagerFillUseCase(shaderManager, vertex_array_object, offset, index_count);
};
