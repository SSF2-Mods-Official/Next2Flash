...
import { $terminateGrid, $gridDataMap } from "../../Grid";
import { $fillBufferIndexes, $fillTypes, $clearFillBufferSetting } from "../../Mesh";
/**
...
export const execute = () => {
    ...
    for (let idx = 0; idx < $fillTypes.length; idx++) {
        if ($gridDataMap.has(idx)) {
            gridData = $gridDataMap.get(idx);
        }
        const type = $fillTypes[idx];
        switch (type) {
            case "fill": // 通常のShapeの塗り
                {
                    const count = $fillBufferIndexes.shift();
                    contextNormalFillUseCase(fillVertexArrayObject, fillOffset, count, gridData);
                    fillOffset += count;
                }
                break;
            case "linear": // 線形グラデーションの塗り
                {
                    const count = $fillBufferIndexes.shift();
                    contextLinearGradientFillUseCase(fillVertexArrayObject, fillOffset, count, gridData);
                    fillOffset += count;
                }
                break;
            case "radial": // 円形グラデーションの塗り
                {
                    const count = $fillBufferIndexes.shift();
                    contextRadialGradientFillUseCase(fillVertexArrayObject, fillOffset, count, gridData);
                    fillOffset += count;
                }
                break;
            case "bitmap": // 画像の塗りつぶし
                {
                    const count = $fillBufferIndexes.shift();
                    contextPatternBitmapFillUseCase(fillVertexArrayObject, fillOffset, count, gridData);
                    fillOffset += count;
                }
                break;
        }
