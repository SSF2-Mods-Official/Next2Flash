...
import { $context, $getDevicePixelRatio } from "../../../WebGLUtil";
/**
 * @type {number[]}
 * @constant
 */
const $STEP = [0.5, 1.05, 1.4, 1.55, 1.75, 1.9, 2, 2.15, 2.2, 2.3, 2.5, 3, 3, 3.5, 3.5];
/**
 ...
 * @protected
 */
export const execute = (texture_object, matrix, blur_x = 4, blur_y = 4, quality = 1, removed = true) => {
    ...
    const attachments = [attachmentObject0, attachmentObject1];
    // 描画元のテクスチャをattachmentObject0に描画
    $context.bind(attachmentObject0);
    $context.reset();
    $context.setTransform(bufferScaleX, 0, 0, bufferScaleY, dx * bufferScaleX, dy * bufferScaleY);
    textureManagerBind0UseCase(texture_object);
    const shaderManager = variantsBlendMatrixTextureShaderService();
    shaderManagerSetMatrixTextureUniformService(shaderManager, texture_object.width, texture_object.height);
    ...
    let textureObject = attachmentObject0.texture;
    ...
    for (let q = 0; q < quality; ++q) {
        if (blur_x > 0) {
            ...
            filterApplyDirectionalBlurFilterUseCase(textureObject, true, bufferBlurX);
        ...
        if (blur_y > 0) {
            ...
            const attachmentObject = attachments[attachmentIndex];
            $context.bind(attachmentObject);
            filterApplyDirectionalBlurFilterUseCase(textureObject, false, bufferBlurY);
            textureObject = attachmentObject.texture;
        }
    }
    blendResetService();
