...
import { $getAtlasAttachmentObject, $clearTransferBounds } from "./AtlasManager";
import { $setReadFrameBuffer, $setDrawFrameBuffer, $getCurrentAttachment, $setAtlasFrameBuffer, $setBitmapFrameBuffer } from "./FrameBufferManager";
import { $setRenderMaxSize, $setWebGL2RenderingContext, $setSamples, $getFloat32Array9, $getArray, $setContext, $setDevicePixelRatio } from "./WebGLUtil";
/**
...
export class Context {
     ...
     * @public
     */
    constructor(gl, samples, device_pixel_ratio = 1) {
        /**
     ...
     */
    fillBackgroundColor() {
        contextFillBackgroundColorService(this.$clearColorR, this.$clearColorG, this.$clearColorB, this.$clearColorA);
    }
    /**
    ...
    gradientFill(type, stops, matrix, spread, interpolation, focal) {
        contextGradientFillUseCase(type, stops, matrix, spread, interpolation, focal);
    }
    /**
    ...
    bitmapFill(pixels, matrix, width, height, repeat, smooth) {
        contextBitmapFillUseCase(pixels, matrix, width, height, repeat, smooth);
    }
    /**
     * @description 線の描画を実行
    ...
    gradientStroke(type, stops, matrix, spread, interpolation, focal) {
        contextGradientStrokeUseCase(type, stops, matrix, spread, interpolation, focal);
    }
    /**
    ...
    bitmapStroke(pixels, matrix, width, height, repeat, smooth) {
        contextBitmapStrokeUseCase(pixels, matrix, width, height, repeat, smooth);
    }
    /**
    ...
    drawDisplayObject(node, x_min, y_min, x_max, y_max, color_transform) {
        ...
        blnedDrawDisplayObjectUseCase(node, x_min, y_min, x_max, y_max, color_transform);
    }
    /**
     ...
     * @public
     */
    applyFilter(node, unique_key, updated, width, height, is_bitmap, matrix, color_transform, blend_mode, bounds, params) {
        this.drawArraysInstanced();
        contextApplyFilterUseCase(node, unique_key, updated, width, height, is_bitmap, matrix, color_transform, blend_mode, bounds, params);
    }
    /**
