...
import { $getCurrentAtlasIndex, $setCurrentAtlasIndex } from "../../AtlasManager";
 ...
 * @protected
 */
export const execute = (node, x_min, y_min, x_max, y_max, color_transform) => {
        ...
        case "copy":
            {
                ...
                node.x / $RENDER_MAX_SIZE, node.y / $RENDER_MAX_SIZE, node.w / $RENDER_MAX_SIZE, node.h / $RENDER_MAX_SIZE, 
        ...
        default:
            {
                ...
                if (matrix[0] !== 1 || matrix[1] !== 0
                    ...
                    const a = $getFloat32Array6(matrix[0], matrix[1], matrix[3], matrix[4], width / 2, height / 2);
                    const b = $getFloat32Array6(1, 0, 0, 1, -node.w / 2, -node.h / 2);
                    const tMatrix = $getFloat32Array6(a[0] * b[0] + a[2] * b[1], a[1] * b[0] + a[3] * b[1], a[0] * b[2] + a[2] * b[3], a[1] * b[2] + a[3] * b[3], a[0] * b[4] + a[2] * b[5] + a[4], a[1] * b[4] + a[3] * b[5] + a[5]);
                    $context.save();
                    ...
                    shaderManagerSetMatrixTextureUniformService(shaderManager, srcTextureObject.width, srcTextureObject.height);
                    ...
                    srcTextureObject = attachmentObject.texture;
                ...
                const dstTextureObject = frameBufferManagerGetTextureFromBoundsUseCase(matrix[6], matrix[7], width, height);
                const attachmentObject = frameBufferManagerGetAttachmentObjectUseCase(width, height, false);
                $context.bind(attachmentObject);
                // ブレンドするテクスチャをバインド
                ...
                shaderManagerSetBlendWithColorTransformUniformService(shaderManager, ct0, ct1, ct2, ct3, ct4, ct5, ct6, ct7);
                shaderManagerDrawTextureUseCase(shaderManager);
                if (currentAttachmentObject) {
                    $context.bind(currentAttachmentObject);
                }
                // ブレンドしたtextureを元の座標に描画
                frameBufferManagerTransferTextureFromRectUseCase(attachmentObject.texture);
                // matrixを元に戻す
                if (offsetX || offsetY) {
                    matrix[6] += offsetX;
                    matrix[7] += offsetY;
