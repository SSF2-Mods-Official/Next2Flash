...
import { $getGradientAttachmentObject, $getGradientLUTGeneratorMaxLength, $rgbIdentityTable, $rgbToLinearTable } from "../../GradientLUTGenerator";
/**
 ...
 * @protected
 */
export const execute = (stops, interpolation) => {
    ...
    for (let begin = 0; begin < stopsLength; begin += maxLength - 1) {
        ...
        const shaderManager = variantsGradientLUTShaderService(end - begin, isLinearSpace);
        gradientLUTSetUniformService(shaderManager, stops, begin, end, table);
        gradientLUTGeneratorFillTextureUseCase(shaderManager, stops[0], stops[stops.length - 5]);
    }
    blendResetService();
    if (currentAttachment) {
        $context.bind(currentAttachment);
    }
    contextBeginNodeRenderingService(scissorBox[0], scissorBox[1], scissorBox[2], scissorBox[3]);
    return gradientAttachmentObject.texture;
};
