 ...
 * @protected
 */
export const execute = (component_x, component_y, mode) => {
    const key = `d${component_x}${component_y}${mode}`;
    if ($collection.has(key)) {
        return $collection.get(key);
    }
    const mediumpLength = mode === 1 ? 3 : 2;
    const shaderManager = new ShaderManager(TEXTURE_TEMPLATE(), DISPLACEMENT_MAP_FILTER_TEMPLATE(mediumpLength, component_x, component_y, mode));
