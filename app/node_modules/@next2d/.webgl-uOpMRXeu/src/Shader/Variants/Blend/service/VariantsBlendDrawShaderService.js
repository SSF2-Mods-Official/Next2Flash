 ...
 * @protected
 */
export const execute = (operation, with_color_transform) => {
    const key = `i${operation}${with_color_transform ? "y" : "n"}`;
    if ($collection.has(key)) {
        return $collection.get(key);
    }
    const shaderManager = new ShaderManager(TEXTURE_TEMPLATE(), BLEND_TEMPLATE(operation, with_color_transform));
