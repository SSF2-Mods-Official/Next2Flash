 ...
 * @protected
 */
export const execute = (is_radial, has_focal_point, spread_method, use_grid) => {
    const key = variantsGradientCreateCollectionKeyService(use_grid, is_radial, has_focal_point, spread_method);
    if ($collection.has(key)) {
        return $collection.get(key);
    }
    const highpLength = (use_grid ? 14 : 5) + 1;
    const fragmentIndex = highpLength - 1;
    const shaderManager = new ShaderManager(FILL_TEMPLATE(highpLength, true, false, use_grid), GRADIENT_TEMPLATE(highpLength, fragmentIndex, is_radial, has_focal_point, spread_method));
