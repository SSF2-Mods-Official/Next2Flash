...
import { $context, $getFloat32Array6, $getDevicePixelRatio, $multiplyMatrices, $poolFloat32Array6 } from "../../WebGLUtil";
/**
 ...
 * @protected
 */
export const execute = (node, unique_key, updated, width, height, is_bitmap, matrix, color_transform, blend_mode, bounds, params) => {
    const currentAttachmentObject = $context.currentAttachmentObject;
    let textureObject = null;
    const scaleX = Math.sqrt(matrix[0] * matrix[0] + matrix[1] * matrix[1]);
    const scaleY = Math.sqrt(matrix[2] * matrix[2] + matrix[3] * matrix[3]);
    const key = $cacheStore.generateFilterKeys(matrix[0], matrix[1], matrix[2], matrix[3]);
    let useCache = false;
    const fKey = $cacheStore.get(unique_key, "fKey");
    if (fKey === key) {
        const cacheTextureObject = $cacheStore.get(unique_key, "fTexture");
        if (updated) {
    ...
    if (!useCache) {
        ...
        const a = $getFloat32Array6(a0, b1, c2, d3, width / 2, height / 2);
        const b = $getFloat32Array6(1, 0, 0, 1, -node.w / 2, -node.h / 2);
        ...
        if (tMatrix[0] !== 1 || tMatrix[1] !== 0 || tMatrix[2] !== 0 || tMatrix[3] !== 1) {
            ...
            $context.reset();
            $context.setTransform(tMatrix[0], tMatrix[1], tMatrix[2], tMatrix[3], tMatrix[4], tMatrix[5]);
            offsetX = tMatrix[4];
            offsetY = tMatrix[5];
            textureManagerBind0UseCase(textureObject);
            // 元の描画をフィルター用のテクスチャに描画
            const shaderManager = variantsBlendMatrixTextureShaderService();
            shaderManagerSetMatrixTextureUniformService(shaderManager, textureObject.width, textureObject.height);
        ...
        for (let idx = 0; params.length > idx;) {
            ...
            switch (type) {
                case 0: // BevelFilter
                    textureObject = filterApplyBevelFilterUseCase(textureObject, matrix, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], Boolean(params[idx++]));
                    break;
                case 1: // BlurFilter
                    textureObject = filterApplyBlurFilterUseCase(textureObject, matrix, params[idx++], params[idx++], params[idx++]);
                    break;
                case 2: // ColorMatrixFilter
                    textureObject = filterApplyColorMatrixFilterUseCase(textureObject, new Float32Array([
                ...
                case 3: // ConvolutionFilter
                    {
                        ...
                        idx += length;
                        textureObject = filterApplyConvolutionFilterUseCase(textureObject, matrix_x, matrix_y, matrix, params[idx++], params[idx++], Boolean(params[idx++]), Boolean(params[idx++]), params[idx++], params[idx++]);
                    }
                    break;
                case 4: // DisplacementMapFilter
                    {
                        ...
                        idx += length;
                        textureObject = filterApplyDisplacementMapFilterUseCase(textureObject, matrix, buffer, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++]);
                    }
                    break;
                case 5: // DropShadowFilter
                    textureObject = filterApplyDropShadowFilterUseCase(textureObject, matrix, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], Boolean(params[idx++]), Boolean(params[idx++]), Boolean(params[idx++]));
                    break;
                case 6: // GlowFilter
                    textureObject = filterApplyGlowFilterUseCase(textureObject, matrix, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], Boolean(params[idx++]), Boolean(params[idx++]));
                    break;
                case 7: // GradientBevelFilter
                    {
                        ...
                        idx += length;
                        textureObject = filterApplyGradientBevelFilterUseCase(textureObject, matrix, distance, angle, colors, alphas, ratios, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], Boolean(params[idx++]));
                    }
                    break;
                case 8: // GradientGlowFilter
                    {
                        ...
                        const ratios = params.subarray(idx, idx + length);
                        idx += length;
                        textureObject = filterApplyGradientGlowFilterUseCase(textureObject, matrix, distance, angle, colors, alphas, ratios, params[idx++], params[idx++], params[idx++], params[idx++], params[idx++], Boolean(params[idx++]));
                    }
                    break;
            }
        }
    ...
    if (textureObject) {
        ...
        blendDrawFilterToMainUseCase(textureObject, color_transform, -offsetX + xMin + matrix[4], -offsetY + yMin + matrix[5]);
    }
    if (!useCache) {
