 ...
 * @protected
 */
export const execute = (texture_object, blur_texture_object, width, height, base_width, base_height, base_offset_x, base_offset_y, blur_width, blur_height, blur_offset_x, blur_offset_y, is_glow, type, knockout, strength, ratios = null, colors = null, alphas = null, color_r1 = 0, color_g1 = 0, color_b1 = 0, color_a1 = 0, color_r2 = 0, color_g2 = 0, color_b2 = 0, color_a2 = 0) => {
    ...
    let gradientTextureObject = null;
    if (isGradient) {
        gradientTextureObject = gradientLUTGenerateFilterTextureUseCase(ratios, colors, alphas);
    }
    if (isInner) {
        $context.reset();
        ...
        shaderManagerSetTextureUniformService(shaderManager, texture_object.width, texture_object.height);
        shaderManagerDrawTextureUseCase(shaderManager);
        if (isGradient && gradientTextureObject) {
            textureManagerBind02UseCase(blur_texture_object, gradientTextureObject, true);
        }
        else {
        ...
        if (isGradient && gradientTextureObject) {
            textureManagerBind012UseCase(blur_texture_object, texture_object, gradientTextureObject, true);
        }
        else {
            textureManagerBind01UseCase(blur_texture_object, texture_object, true);
        }
    }
    ...
    const shaderManager = variantsBitmapFilterShaderService(transformsBase, transformsBlur, is_glow, type, knockout, appliesStrength, isGradient);
    shaderManagerSetBitmapFilterUniformService(shaderManager, width, height, base_width, base_height, base_offset_x, base_offset_y, blur_width, blur_height, blur_offset_x, blur_offset_y, is_glow, strength, color_r1, color_g1, color_b1, color_a1, color_r2, color_g2, color_b2, color_a2, transformsBase, transformsBlur, appliesStrength, isGradient);
    shaderManagerDrawTextureUseCase(shaderManager);
    const textureObject = attachmentObject.texture;
