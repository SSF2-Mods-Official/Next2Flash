...
import { $offset, $intToR, $intToG, $intToB } from "../../../Filter";
import { $getDevicePixelRatio, $context } from "../../../WebGLUtil";
/**
 * @type {number}
 * @private
 */
 ...
 * @protected
 */
export const execute = (texture_object, matrix, distance = 4, angle = 45, highlight_color = 0xffffff, highlight_alpha = 1, shadow_color = 0, shadow_alpha = 1, blur_x = 4, blur_y = 4, strength = 1, quality = 1, type = 0, knockout = false) => {
    ...
    shaderManagerDrawTextureUseCase(shaderManager);
    blendResetService();
    const bevelBaseTextureObject = attachmentObject.texture;
    if (currentAttachmentObject) {
        $context.bind(currentAttachmentObject);
    }
    const blurTextureObject = filterApplyBlurFilterUseCase(bevelBaseTextureObject, matrix, blur_x, blur_y, quality, false);
    ...
    const textureObject = filterApplyBitmapFilterUseCase(texture_object, blurTextureObject, width, height, baseWidth, baseHeight, baseTextureX, baseTextureY, blurWidth, blurHeight, blurTextureX, blurTextureY, false, typeName, knockout, strength, null, null, null, $intToR(highlight_color, highlight_alpha, true), $intToG(highlight_color, highlight_alpha, true), $intToB(highlight_color, highlight_alpha, true), highlight_alpha, $intToR(shadow_color, shadow_alpha, true), $intToG(shadow_color, shadow_alpha, true), $intToB(shadow_color, shadow_alpha, true), shadow_alpha);
