package captainfalcon_fla
{
    import flash.display.MovieClip;

    public dynamic class FSmash_51 extends MovieClip
    {

        public var attackBox:MovieClip;
        public var hitBox:MovieClip;
        public var hitBox2:MovieClip;
        public var hitBox3:MovieClip;
        public var itemBox:MovieClip;
        public var self:CaptainExt;
        public var xframe:String;

        public function FSmash_51()
        {
            super();
            addFrameScript(0, this.frame1, 6, this.frame7, 46, this.frame47, 47, this.frame48, 49, this.frame50, 50, this.frame51, 69, this.frame70);
        }

        public function effects():void
        {
            this.self.attachEffect("global_dust_heavy", {
                "x":this.self.flipX(5),
                "y":3,
                "scaleX":-0.5,
                "scaleY":-0.5
            });
        }

        internal function frame1():*
        {
            this.self = (SSF2API.getCharacter(this) as CaptainExt);
            this.xframe = null;
        }

        internal function frame7():*
        {
            this.xframe = "charging";
            this.self.createTimer(4, -1, this.effects);
        }

        internal function frame47():*
        {
            this.self.stancePlayFrame("charging");
        }

        internal function frame48():*
        {
            this.xframe = "attack";
            this.self.destroyTimer(this.effects);
        }

        internal function frame50():*
        {
            this.self.setXSpeed(10, false);
            this.self.playSound("cfalcon_smashstart");
            this.self.attachEffect("global_dust_heavy");
        }

        internal function frame51():*
        {
            this.self.attachEffect("wind_wave", {
                "x":this.self.flipX(55),
                "y":-35,
                "scaleX":0.8,
                "scaleY":0.8
            });
            this.self.attachEffect("global_dust_blast", {
                "x":this.self.flipX(60),
                "y":-35,
                "parentLock":true
            });
            this.self.playAttackSound(1);
            this.self.playVoiceSound(1);
            this.self.addEffectToList(this.self.attachEffect("trail_cfalcon_fsmash", {
                "scaleX":1.15,
                "scaleY":1.15,
                "parentLock":true,
                "syncHitStun":true
            }));
            this.self.clearEffectsOnStateChange();
        }

        internal function frame70():*
        {
            this.self.endAttack();
        }


    }
}

